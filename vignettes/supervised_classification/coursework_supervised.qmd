...
```{python}
import sys
sys.path.append(r"C:\Users\JWWeb\OneDrive - University of Exeter\Documents\applications-jw1574")
```
...
fig.update_layout(title="Group Summary Statistics")
fig
```
...
```{python}
tables = pd.read_html('../../data_cache/vignettes/supervised_classification/scatterplot.html')
df = tables[0]  # first table in the file
```
...
```{python}
from course.supervised_classification.metrics import metric_report_lda 
